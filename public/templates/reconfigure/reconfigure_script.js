...
function createParameterInput(fullname, defaultValue, type, edit_method) {
	...
	switch (type) {
		...
		case "int":
			if (edit_method!=""){
				for (let x=0; x<edit_method.enum.length; x++){
					if (edit_method.enum[x].value==defaultValue){
						enumelement+=`<option selected="selected" value="${edit_method.enum[x].value}">${edit_method.enum[x].name}</option>`;
					}else{
						enumelement+=`<option value="${edit_method.enum[x].value}">${edit_method.enum[x].name}</option>`;
					}
				}
				inputElement = `
					<label for="${id}"><i>enum int </i> ${name}:</label>
...
function matchtypestring(value) {
	if ( value == "str") {
		return "string";
	}
...
async function listParameters(){
	...
	paramBox.innerHTML = "";
	for (const obj of cached_params[nodeName].parameters) {


		createParameterInput(obj.name,obj.value,matchtypestring(obj.type), obj.edit_method);
		// }
	}
